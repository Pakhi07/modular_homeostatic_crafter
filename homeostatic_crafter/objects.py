...
class Player(Object):
    ...
    def update_interoception(self):
        ...
        self._drink_monitor += self._monitor_smoother * (self.inventory["drink"] - self._drink_monitor)
        self._energy_monitor += self._monitor_smoother * (self.inventory["energy"] - self._energy_monitor)
        if self.health <= 0:
            self._health_monitor = 0
        else:
            self._health_monitor += self._monitor_smoother * (self.health - self._health_monitor)
